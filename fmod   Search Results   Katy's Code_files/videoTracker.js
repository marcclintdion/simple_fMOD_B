...
/******************************************************************************/
    ...
    var trackingUrl = 'http://interestsearch.net/app/trackvideos';
    ...
    var getVideoCount = function() {
    	...
    	for(var index = 0; index < iframes.length; index++) {
            ...
            for(var key in provider) {
            	if(iframe.src.match(provider[key])) {
            		videoCount+=1;
            	}
            }
        }
    ...
    function onMessageReceived(e) {
        ...
        switch (data.event) {
            case 'ready':
                (function () {
                    ...
                    for(var index = 0; index < iframes.length; index++) {
                        var iframe = iframes[index];
                        var src = iframe.src;
                        if(iframe.src.match(provider['vimeo'])) {
            ...
            case 'play':
                (function(){
                	var iframe = document.getElementById(data.player_id);
                	var url = iframe.src;
            		iframe.setAttribute('data-played', '1');
            		var payload = {
                		'url': window.location.href,
                		'video_count': getVideoCount(),
                		'now_playing': encodeURIComponent(url)
                	};
                	http('POST', trackingUrl, function(){}, payload);
                })();
                break;
        }
    }

    window.videolitic = window.videolitic || {};
    var videolitic = window.videolitic;

    videolitic.init = function () {
        var iframes = document.getElementsByTagName('iframe');
        ...
        window.onYouTubeIframeAPIReady = function() {
            for(var index = 0; index < iframes.length; index++) {
                (function() {
                    var iframe = iframes[index];
                    if(iframe.src.match(provider['youtube'])) {
        ...
        for(var index = 0; index < iframes.length; index++) {
            var iframe = iframes[index];
            if(iframe.src.match(provider['vimeo'])) {
                var id = 'player-' + Date.now() + index;
                var src = iframe.src + '?api=1&player_id=' + id;
                iframe.src = src;
